...
const LEVELS={
 1:{name:'Level 1: Safe Password City',target:3600,speed:2.15,spawn:1500,traffic:3,mission:'The car drives forward automatically. Steer left or right, avoid traffic, and answer 5 simple cyber-safety questions.'},
 2:{name:'Level 2: Phishing Highway',target:4600,speed:2.55,spawn:1350,traffic:4,mission:'The road is a little faster. Avoid traffic and answer 5 phishing and scam questions. Complete Level 1 first to unlock this.'},
 3:{name:'Level 3: Dark Web Rush',target:5600,speed:2.95,spawn:1220,traffic:5,mission:'Final mission: faster traffic with 5 malware, privacy, and account-safety questions.'}
};
const QUESTIONS={
...
let W=0,H=0,renderScale=1,road={left:0,right:0,topLeft:0,topRight:0},player={x:0,y:0,w:70,h:105,vx:0,target:null};
let currentQuestion=null,hintUsed=false; const HINT_UNLOCK_SCORE=1200;
document.getElementById('levelName').textContent=L.name;document.getElementById('missionText').textContent=L.mission;
function lowPower(){return W<720||renderScale<window.devicePixelRatio}
function resize(){const box=document.getElementById('gameArea').getBoundingClientRect();renderScale=Math.min(window.devicePixelRatio||1,box.width<720?1.1:1.35);canvas.width=Math.floor(box.width*renderScale);canvas.height=Math.floor(box.height*renderScale);canvas.style.width=box.width+'px';canvas.style.height=box.height+'px';ctx.setTransform(renderScale,0,0,renderScale,0,0);W=box.width;H=box.height;road.left=W*.08;road.right=W*.92;road.topLeft=W*.32;road.topRight=W*.68;player.w=Math.max(54,Math.min(78,W*.075));player.h=player.w*1.52;player.y=H-player.h-36;if(!player.x)player.x=W/2-player.w/2;clampPlayer();draw()}window.addEventListener('resize',resize);resize();
...
function startEngine(){if(engine)return;engine=setInterval(()=>{if(running&&!paused)tone((48+Math.random()*24)*manualGear,.045,'sawtooth',.008)},190)}
function startMusic(){if(music)return;music=setInterval(()=>{if(running&&!paused){const seq=[196,247,294,247,330,294,247,220];tone(seq[ambientBeat%seq.length],.08,'triangle',.01);if(ambientBeat%6===0)tone(98,.07,'sine',.01);ambientBeat++;}},420)}
...
function drawRoad(){ctx.save();ctx.fillStyle='#071022';ctx.fillRect(0,0,W,H);let sky=ctx.createLinearGradient(0,0,0,H);sky.addColorStop(0,'#111b46');sky.addColorStop(.65,'#071022');sky.addColorStop(1,'#050612');ctx.fillStyle=sky;ctx.fillRect(0,0,W,H);const buildings=lowPower()?10:18;for(let i=0;i<buildings;i++){let x=(i*112+roadOffset*.055)%W,h=60+((i*41)%105);ctx.fillStyle=i%2?'rgba(56,232,255,.12)':'rgba(255,79,216,.10)';ctx.fillRect(x,H*.12-h,38,h);if(!lowPower()){ctx.fillStyle='rgba(255,255,255,.08)';ctx.fillRect(x+8,H*.12-h+14,6,h-24)}}ctx.beginPath();ctx.moveTo(road.topLeft,0);ctx.lineTo(road.topRight,0);ctx.lineTo(road.right,H);ctx.lineTo(road.left,H);ctx.closePath();let rg=ctx.createLinearGradient(0,0,0,H);rg.addColorStop(0,'#31343d');rg.addColorStop(1,'#151515');ctx.fillStyle=rg;ctx.fill();ctx.lineWidth=lowPower()?6:8;ctx.strokeStyle='#ffd84a';ctx.beginPath();ctx.moveTo(road.topLeft,0);ctx.lineTo(road.left,H);ctx.moveTo(road.topRight,0);ctx.lineTo(road.right,H);ctx.stroke();ctx.strokeStyle='rgba(255,255,255,.75)';ctx.lineWidth=lowPower()?4:5;ctx.setLineDash([34,30]);ctx.lineDashOffset=-roadOffset;for(let p of [.33,.5,.67]){ctx.beginPath();ctx.moveTo(road.topLeft+(road.topRight-road.topLeft)*p,0);ctx.lineTo(road.left+(road.right-road.left)*p,H);ctx.stroke()}ctx.setLineDash([]);ctx.restore()}
function drawCar(x,y,w,h,c1,c2,enemy=false){ctx.save();ctx.translate(x+w/2,y+h/2);if(!lowPower()){ctx.shadowColor=enemy?'rgba(255,36,85,.55)':'rgba(56,232,255,.55)';ctx.shadowBlur=14}let grd=ctx.createLinearGradient(-w/2,-h/2,w/2,h/2);grd.addColorStop(0,c1);grd.addColorStop(1,c2);roundRect(-w/2,-h/2,w,h,18,grd);roundRect(-w*.27,-h*.31,w*.54,h*.34,12,enemy?'#ff9eb0':'rgba(5,12,28,.72)');ctx.shadowBlur=0;ctx.fillStyle='#05070e';roundRect(-w*.62,-h*.26,w*.18,h*.28,8);roundRect(w*.44,-h*.26,w*.18,h*.28,8);roundRect(-w*.62,h*.16,w*.18,h*.28,8);roundRect(w*.44,h*.16,w*.18,h*.28,8);ctx.fillStyle=enemy?'#ffccd5':'#eaffff';roundRect(-w*.25,h*.34,w*.16,h*.06,5);roundRect(w*.09,h*.34,w*.16,h*.06,5);ctx.restore()}
function roundRect(x,y,w,h,r,fill){ctx.beginPath();ctx.roundRect(x,y,w,h,r);ctx.fillStyle=fill;ctx.fill()}
function laneX(y,w,laneRatio){const b=roadWidthAt(y);return b.l+(b.r-b.l)*laneRatio-w/2}
function spawn(type){let b=roadWidthAt(-20),w=type==='enemy'?64:42,h=type==='enemy'?98:42,laneRatio=[.25,.5,.75][Math.floor(Math.random()*3)],x=type==='enemy'?laneX(-20,w,laneRatio):b.l+30+Math.random()*(b.r-b.l-100);entities.push({type,x,y:-120,w,h,laneRatio,speed:1+Math.random()*.7,kind:['💰','🛡️','⚡','💾'][Math.floor(Math.random()*4)]})}
function loop(t){if(!running||paused)return;let dt=Math.min(32,t-last);last=t;updateManualGear(dt);let currentSpeed=L.speed*nitro*manualGear;roadOffset+=dt*(currentSpeed*.22);distance+=dt*currentSpeed/22;score+=Math.floor(currentSpeed);spawnTimer+=dt;itemTimer+=dt;if(spawnTimer>L.spawn){spawn('enemy');if(progress.level>1&&Math.random()<.22)spawn('enemy');spawnTimer=0}if(itemTimer>1350){spawn('item');itemTimer=0}let input=(keys.arrowright||keys.d?1:0)-(keys.arrowleft||keys.a?1:0);
...
function updateManualGear(dt){let old=manualGear;if(keys.arrowup||keys.w)manualGear+=dt*.00055;else if(keys.arrowdown||keys.s)manualGear-=dt*.0008;else manualGear+=(1-manualGear)*.018;manualGear=Math.max(.55,Math.min(1.45,manualGear));if(old<1.18&&manualGear>=1.18)sound('speed');if(old>.82&&manualGear<=.82)sound('brake')}
function updateEntities(dt){for(let i=entities.length-1;i>=0;i--){let e=entities[i];let sp=(L.speed*nitro*manualGear*dt/5.8)*e.speed;e.y+=sp;let b=roadWidthAt(e.y+e.h);if(e.type==='enemy'){e.x=laneX(e.y+e.h*.5,e.w,e.laneRatio)}else{e.x=Math.max(b.l+15,Math.min(b.r-e.w-15,e.x))}if(rect(player,e)){if(e.type==='item'){collect(e);entities.splice(i,1);continue}else if(invuln<=0){crash();entities.splice(i,1);continue}}if(e.y>H+150)entities.splice(i,1)}}
function rect(a,b){return a.x<b.x+b.w&&a.x+a.w>b.x&&a.y<b.y+b.h&&a.y+a.h>b.y}function collect(e){coins+=e.kind==='💰'?5:12;score+=150;if(e.kind==='🛡️')shield=1;if(e.kind==='⚡'){nitro=NITRO_BOOST;sound('nitro')}else sound('coin')}
...
function checkQuestionGate(){let next=(questionsAnswered+1)/5*L.target;if(questionsAnswered<5&&distance>=next)showQuestion(false);if(distance>=L.target&&questionsAnswered>=5){if(correct>=3)win();else gameOver('You reached the finish, but you need at least 3 correct cyber answers to unlock the next level.')}}
function showQuestion(){
  paused=true;cin.classList.add('show');setTimeout(()=>cin.classList.remove('show'),650);sound('checkpoint');
