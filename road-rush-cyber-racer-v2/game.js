...
function resize() {
  ...
  player.w = Math.max(62, Math.min(82, W * .075));
...
function hold(id, key) {
  ...
  if (!b) return;
  b.onmousedown = b.ontouchstart = e => { keys[key] = true; e.preventDefault(); };
  b.onmouseup = b.onmouseleave = b.ontouchend = () => keys[key] = false;
}
...
function drawCar(x, y, w, h, c1, c2, enemy = false) {
  ...
  ctx.shadowColor = enemy ? 'rgba(255,36,85,.55)' : 'rgba(56,232,255,.55)'; ctx.shadowBlur = 18;
...
function spawn(type) {
  ...
  let w = type === 'enemy' ? 64 : 42, h = type === 'enemy' ? 98 : 42;
  entities.push({ type, x, y: -120, w, h, speed: 1 + Math.random() * .7, kind: ['💰', '🛡️', '⚡', '💾'][Math.floor(Math.random() * 4)] });
}
...
function loop(t) {
  ...
  last = t;
  updateManualGear(dt);
  let currentSpeed = L.speed * nitro * manualGear;
  ...
  if (itemTimer > 1350) { spawn('item'); itemTimer = 0; }

  let input = (keys.arrowright ? 1 : 0) - (keys.arrowleft ? 1 : 0);
...
function updateManualGear(dt) {
  let old = manualGear;
  if (keys.a) manualGear += dt * .00055;
  else if (keys.d) manualGear -= dt * .0008;
  else manualGear += (1 - manualGear) * .018;
  manualGear = Math.max(.55, Math.min(1.45, manualGear));
